          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              ...
              style={{
                ...
                borderWidth: 1, borderColor: statusFilter === item.key ? Colors.primary : Colors.border,
                marginRight: 8,
              }}
            >
        ...
        {priorityFilters.map((pf) => (
          <TouchableOpacity
            ...
            style={{
              ...
              borderWidth: 1, borderColor: priorityFilter === pf.key ? pf.color : Colors.border,
            }}
          >
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4, flexWrap: 'wrap', gap: 8 }}>
          <View style={{ paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, backgroundColor: PRIORITY_COLORS[item.priority] + '20' }}>
      ...
      {item.assigned_user && (
        <View style={{ marginLeft: 8 }}>
          {item.assigned_user.avatar_url ? (
            <Image source={{ uri: resolveUrl(item.assigned_user.avatar_url)! }} style={{ width: 28, height: 28, borderRadius: 14 }} />
          ) : (
            <View style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.primary, justifyContent: 'center', alignItems: 'center' }}>
              <Text style={{ fontSize: 11, fontWeight: '700', color: '#fff' }}>{getInitials(item.assigned_user.name)}</Text>
            </View>
          )}
        </View>
      )}
